 ...
 *
 * This component provides a modal interface for editing an existing project.
 * Users can edit the project name, active mark, and assigned users
 ...
 * - Editing project by submitting the new project name, active status, and selected users to the backend.
 * - Handling errors such as project duplication with appropriate messaging.
 * - Displaying success messages upon successful project edits and refreshes the page.
 *
 * Props:
 ...
 */
const EditProject = ({ isOpen, toggle, projectToEdit }) => {
  // State variables
  const [selectAll, setSelectAll] = useState(false); // State to manage "Select All" checkbox for users
  const [selectedUsers, setSelectedUsers] = useState(projectToEdit.selectedUsers); // State to store selected users (user IDs), set to initial
  const [isActive, setIsActive] = useState(projectToEdit.active); // State for the project's "active" status, set to original
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        setIsActive(projectToEdit.isActive); // set default active status
  ...
  /**
   * handleSubmit - Handles the form submission for editing an existing project.
   *
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const updatedProject = {
        ...
        users: selectedUsers, // Include selected user IDs(ones that are currently selected in checkbox)
      };
      ...
      const response = await client.patch('/project', updatedProject);
          ...
          <Row>
            <FormGroup className="col-md-5">
              ...
              <Input
                ...
                id="projectName"
                required
                readOnly // don't change project names, its how we edit projects
              />
            </FormGroup>
            <Col>
...
EditProject.propTypes = {
  ...
  projectToEdit: PropTypes.object.isRequired, //ensure that projectToEdit is a required object
};
